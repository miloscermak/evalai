...
- [x] **Ořez `interpretation` opraven (2026-08-26):** prompt má teď tvrdý limit 600 znaků + explicitní „musí končit dokončenou větou", tool schema `maxLength` sníženo na 600 a tvrdý `slice(0,700)` nahrazen funkcí `trimToSentence()` — ořízne na konci poslední celé věty, jinak na hranici slova s výpustkou. Nikdy neskončí uprostřed slova. Pozn.: když model přeteče hodně, sentence-ořez může obětovat celou poslední větu — proto ta prevence v promptu.
